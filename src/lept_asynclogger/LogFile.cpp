...
lept_base::LogFile::LogFile(const std::string &filename, int flush_every_N)
...
int lept_base::LogFile::flush()
{
    MutexLockGuard lock(*p_mutex_);
    return p_file_->flush();
}

size_t lept_base::LogFile::append(const char *logline, size_t len)
{
    MutexLockGuard lock(*p_mutex_);
...
size_t lept_base::LogFile::append_unlocked(const char *logline, size_t len)
{
